...
// if everything is ok, try to upload file
      ...
      $query="Select movie_id from movies where name=?";
      if ($stmt1 = $db->prepare($query)) {
  
            $stmt1->bind_param("s",$name);
   
            $stmt1->execute();
            $stmt1->bind_result($movie_id);
            $stmt1->fetch();
            echo $movie_id;
            $stmt1->close();
      }else {
      ...
      if( $conn->query("UPDATE movie_reviews SET comment='$comment', rating='$rating' where movie_id='$movie_id'")==true){
...
<body>
  <form method="post" action="moviecomment_page.php" enctype="multipart/form-data">
    <div id="a1">
    Name:
